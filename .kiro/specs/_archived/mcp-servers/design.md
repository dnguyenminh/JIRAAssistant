...
Internal MCP Server là một MCP server tích hợp sẵn trong Backend_Server, expose toàn bộ chức năng ứng dụng (navigation, scan, analysis, chat, settings, user management, knowledge graph, dashboard, diagram generation) dưới dạng MCP tools. Khác với external MCP servers (chạy process riêng, giao tiếp qua stdio/JSON-RPC), Internal MCP Server chạy in-process và gọi trực tiếp service layers.
...
graph TB
    subgraph "Backend Server (Ktor 3.4.0)"
        ...
        subgraph "Internal MCP Server"
            ...
            IMTE --> TR[ToolRegistry<br>32+ tool definitions]
...
- `GET /{id}/tools` → `bridge.getAggregatedTools()` (trả về 32 internal tools)
...
| 31 | `create_drawio_diagram` | DIAGRAM | xml, filename, diagramType?, format? | VIEW_ANALYSIS | 6.103a |
| 32 | `list_drawio_diagrams` | DIAGRAM | — | VIEW_ANALYSIS | 6.103b |
