...
**User Story:** Là một AI agent kết nối qua MCP, tôi muốn có thể điều khiển toàn bộ giao diện và chức năng của ứng dụng (điều hướng trang, quản lý scan, phân tích ticket, chat, cài đặt, quản lý user, knowledge graph, tạo diagram) — để tự động hóa workflow mà không cần tương tác thủ công qua UI.
...
**6.103a** THE Internal_MCP_Server SHALL cung cấp tool `create_drawio_diagram` nhận tham số `xml` (required, mxGraphModel XML content), `filename` (required, tên file không có extension), `diagramType` (optional, enum: flowchart/sequence/class/er/architecture/network/mindmap/state/activity/component/deployment/custom), `format` (optional, enum: drawio/png/svg/pdf, default drawio). Tool validate XML structure (phải có mxGraphModel root, cells id=0 và id=1, không chứa XML comments), ghi file .drawio vào thư mục `diagrams/`, và trả về file path + metadata. RBAC: yêu cầu Reader+ permission.

**6.103b** THE Internal_MCP_Server SHALL cung cấp tool `list_drawio_diagrams` không nhận tham số. Tool trả về danh sách files .drawio trong thư mục `diagrams/` gồm: name, sizeBytes, lastModified. RBAC: yêu cầu Reader+ permission.
