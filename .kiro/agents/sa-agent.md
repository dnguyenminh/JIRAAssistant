...
2. Verify `documents/{TICKET-KEY}/BRD.md` exists — read it with `skipPruning=true`.
3. Verify `documents/{TICKET-KEY}/FSD.md` exists — read it with `skipPruning=true`.
4. If either file is missing, inform the user: "Cần có BRD và FSD trước khi tạo TDD. Hãy chạy ba-agent trước."
...
**Diagram Embedding Rule:** Every `.drawio` file MUST have a corresponding `![...](diagrams/....png)` reference in TDD.md.
...
3. Use `mcp_markdown_exporter_local_export_docx` with `file_name`: `TDD-{TICKET-KEY}.docx`.
4. Copy exported DOCX to `documents/{TICKET-KEY}/TDD-{TICKET-KEY}.docx`.
