...
describe("detectProjectType", () => {
    ...
    it("detects npm-typescript when package.json and tsconfig.json exist", () => {
        const dir = makeTempDir();
        fs.writeFileSync(path.join(dir, "package.json"), JSON.stringify({ name: "test-pkg", dependencies: {} }));
    ...
    it("detects Spring Boot framework from build.gradle.kts", () => {
        const dir = makeTempDir();
        fs.writeFileSync(path.join(dir, "build.gradle.kts"), `plugins { id("org.springframework.boot") }
dependencies {
