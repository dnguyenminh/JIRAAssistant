...
export const DEFAULT_METADATA_PATH = path.resolve(process.cwd(), ".analysis/code-intelligence/index-metadata.json");
// ---------------------------------------------------------------------------
// Public functions
// ---------------------------------------------------------------------------
/**
...
export function readMetadata(metadataPath) {
    ...
    try {
        ...
        process.stderr.write(`[Code-Index] ERROR: Corrupted metadata — ${resolvedPath} — ${err.message}\n`);
    ...
    if (!isValidMetadata(parsed)) {
        process.stderr.write(`[Code-Index] ERROR: Corrupted metadata — ${resolvedPath} — missing or invalid "version" or "files" field\n`);
        safeDelete(resolvedPath);
        return null;
    }
    return parsed;
 ...
 *   relative to `process.cwd()`.
 */
export function writeMetadata(metadata, metadataPath) {
    const resolvedPath = metadataPath ?? DEFAULT_METADATA_PATH;
    ...
    try {
        ...
        process.stderr.write(`[Code-Index] ERROR: Metadata write failed — ${resolvedPath} — ${err.message}\n`);
