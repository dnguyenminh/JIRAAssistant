 ...
 */
export function parseFile(filePath, language, moduleName) {
...
// ---------------------------------------------------------------------------
function parseTypeScript(filePath, content, language, moduleName) {
...
function extractTsParameters(params, sourceFile) {
    return params.map((p) => ({
...
// ---------------------------------------------------------------------------
function parseKotlin(filePath, content, moduleName) {
    ...
    const classRe = /(public|private|internal|protected|open|abstract|data|sealed)?\s*(class|object|interface|enum class)\s+(\w+)(?:\s*:\s*([\w.]+))?(?:\s*,\s*([\w., ]+))?/;
    const funRe = /(public|private|internal|protected|override|open|suspend)?\s*fun\s+(\w+)\s*\(([^)]*)\)(?:\s*:\s*(\w+))?/;
...
// ---------------------------------------------------------------------------
function parseJava(filePath, content, moduleName) {
    const lines = content.split("\n");
    let packageName = "";
    const imports = [];
    const classes = [];
    ...
    const pkgRe = /^package\s+([\w.]+);/;
    const importRe = /^import\s+([\w.*]+);/;
    const classRe = /(public|private|protected)?\s*(abstract\s+)?(class|interface|enum)\s+(\w+)(?:\s+extends\s+(\w+))?(?:\s+implements\s+([\w, ]+))?/;
    ...
    for (const line of lines) {
        ...
        if (methodMatch) {
            ...
            if (["class", "interface", "enum"].includes(returnType))
                continue;
...
// ---------------------------------------------------------------------------
function parsePython(filePath, content, moduleName) {
    const lines = content.split("\n");
    const imports = [];
    const classes = [];
    const functions = [];
...
// ---------------------------------------------------------------------------
function parseGo(filePath, content, moduleName) {
    let packageName = "";
    const imports = [];
    const classes = [];
    const functions = [];
...
// ---------------------------------------------------------------------------
function parseRust(filePath, content, moduleName) {
    const imports = [];
    const classes = [];
    const functions = [];
    // Use statements
...
// ---------------------------------------------------------------------------
function parseCSharp(filePath, content, moduleName) {
    let packageName = "";
    const imports = [];
    const classes = [];
    const functions = [];
    ...
    // Classes / interfaces
    const classRe = /(public|private|protected|internal)?\s*(abstract\s+)?(class|interface)\s+(\w+)(?:\s*:\s*([\w, ]+))?/g;
    let classMatch;
    ...
    while ((methodMatch = methodRe.exec(content)) !== null) {
        ...
        if (["class", "interface", "enum", "namespace"].includes(returnType))
            continue;
...
// ---------------------------------------------------------------------------
function parseYaml(filePath, content, moduleName) {
    const classes = [];
    const keyRe = /^(\w[\w-]*):/gm;
    let match;
...
function parseXml(filePath, content, moduleName) {
    const classes = [];
...
function parseJson(filePath, content, moduleName) {
    const classes = [];
    try {
...
function parseProperties(filePath, content, moduleName) {
    const classes = [];
    const propRe = /^([\w.]+)\s*=/gm;
    let match;
...
// ---------------------------------------------------------------------------
function parseSql(filePath, content, moduleName) {
    const classes = [];
    // Tables
...
// ---------------------------------------------------------------------------
function parseFallback(filePath, content, language, moduleName) {
    const imports = [];
 ...
 */
function parseParamList(raw, typeSeparator) {
    if (!raw.trim())
        return [];
    return raw
        .split(",")
