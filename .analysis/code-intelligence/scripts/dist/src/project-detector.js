 ...
 */
function countSourceFiles(dir, maxDepth, currentDepth = 0) {
    const counts = {};
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            ...
            const childCounts = countSourceFiles(path.join(dir, entry.name), maxDepth, currentDepth + 1);
 ...
 */
function getPrimaryLanguage(counts) {
    let best = null;
    let bestCount = 0;
 ...
 */
function detectFramework(content, patterns) {
 ...
 */
function isNpmTypeScript(rootDir) {
 ...
 */
function hasTypeScriptFiles(dir, maxDepth, currentDepth) {
    if (currentDepth > maxDepth)
        return false;
    let entries;
    try {
 ...
 */
function findCsprojFile(rootDir) {
    try {
        const entries = fs.readdirSync(rootDir);
...
export function detectProjectType(rootDir) {
    ...
    process.stdout.write(`[Code-Index] INFO: Project detected — type=${projectType}, language=${primaryLanguage}, framework=${framework}, buildFile=${buildFile}\n`);
    return result;
}
