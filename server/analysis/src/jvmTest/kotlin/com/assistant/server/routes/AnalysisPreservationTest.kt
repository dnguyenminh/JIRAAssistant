...
class AnalysisPreservationTest {
    ...
    fun `runAnalysis calls orchestrator analyzeTicket and returns result`() {
        ...
        assertTrue(
            source.contains("return result") || source.contains("return "),
    ...
    fun `runAnalysis signature includes ticketId orchestrator and forceReanalyze`() {
        ...
        assertTrue(
            source.contains("ticketId: String") && source.contains("orchestrator: AIOrchestrator") ||
                source.contains("ticketId: String, orchestrator: AIOrchestrator"),
    ...
    fun `runAnalysis fires status phases in correct order`() {
        ...
        // Note: KB_SYNCING is now conditional (inside if block), so we check
        // the core phases that always fire in order
        val phases = listOf(
        ...
        for (phase in phases) {
            ...
            assertTrue(
                ...
                "Phase '$phase' must appear after previous phase in runAnalysis(). " +
                    ...
                    "analyzeTicket → COMPLETE"
            )
            lastIdx = idx
        }

        // KB_SYNCING should exist somewhere in the function (conditional)
        assertTrue(
            fnBody.contains("AnalysisPhase.KB_SYNCING"),
            "KB_SYNCING phase should exist in runAnalysis()"
        )
    }
