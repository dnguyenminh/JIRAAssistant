...
internal suspend fun RoutingContext.handleGetHistory(chatRepo: ChatRepository) {
    ...
    val messages = if (!convId.isNullOrBlank()) {
        chatRepo.getHistoryByConversation(userId, convId, page, size)
    } else {
        chatRepo.getHistory(userId, page, size)
    }
    val total = if (!convId.isNullOrBlank()) {
        chatRepo.getHistoryCountByConversation(userId, convId)
    } else {
        chatRepo.getHistoryCount(userId)
    }
...
internal fun Route.chatConversationRoutes(
    ...
    delete("/conversations/{id}") {
        ...
        chatRepo.deleteHistoryByConversation(id)
        convRepo.delete(id)
