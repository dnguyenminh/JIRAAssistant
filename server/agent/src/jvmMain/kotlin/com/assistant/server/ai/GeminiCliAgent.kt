...
/**
 * AI Agent that delegates to the Gemini CLI binary.
 * Spawns `gemini` process per request, sends prompt via stdin,
...
class GeminiCliAgent(
    ...
    companion object {
        private const val TIMEOUT_MS = 240_000L // Req 7.1: increased from 120s to 240s
    }

    private val logger = org.slf4j.LoggerFactory.getLogger(GeminiCliAgent::class.java)
    ...
    override suspend fun analyze(
        ...
        try {
            ...
            logger.info("Gemini CLI call: model=$model, prompt=${fullPrompt.length} chars")
            ...
            if (result.exitCode == 0 && result.stdout.isNotBlank()) {
                logger.info("Gemini CLI success: response=${result.stdout.length} chars")
                AIResult.Success(result.stdout.trim())
            } else {
                val err = result.stderr.ifBlank { "Empty response" }
                logger.warn("Gemini CLI failed: exit=${result.exitCode}, stderr=$err")
                AIResult.Failure("Gemini CLI error (exit ${result.exitCode}): $err")
            }
        } catch (e: Exception) {
            logger.error("Gemini CLI exception: ${e.message}", e)
            AIResult.Failure("Gemini CLI error: ${e.message}")
        }
    }

    /**
     * Test connectivity by running `gemini --version`.
    ...
    override fun getAgentName(): String = "Gemini CLI - $model"
