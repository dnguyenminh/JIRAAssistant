...
class AttachmentCuratorPropertyTest {
    ...
    @Test
    fun `preview never exceeds 5000 chars or 8000 for req docs`() = runTest {
        checkAll(PropTestConfig(iterations = 100),
            ...
            result.forEach { att ->
                if (att.isRequirementDoc) {
                    assertTrue(att.preview.length <= CurationConfig.MAX_REQUIREMENT_DOC_PREVIEW_CHARS,
                        "Req doc preview ${att.preview.length} > ${CurationConfig.MAX_REQUIREMENT_DOC_PREVIEW_CHARS}")
                } else {
                    assertTrue(att.preview.length <= CurationConfig.MAX_ATTACHMENT_PREVIEW_CHARS,
                        "Preview ${att.preview.length} > ${CurationConfig.MAX_ATTACHMENT_PREVIEW_CHARS}")
                }
            }
        }
    }
