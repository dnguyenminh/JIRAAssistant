...
class IncrementalBuildErrorResiliencePropertyTest {
    ...
    /**
     * Property 2: For any project with N tickets (1..30), when
     * saveGraphData() always throws, the scan SHALL still reach
     * COMPLETED with processedCount == totalTickets.
     * **Validates: Requirements 1.3**
     */
    @Test
    fun `scan completes despite graph build failures`(): Unit = runBlocking {
        checkAll(
            ...
            val scanStateRepo = InMemoryScanStateForResilience()
            val scanLogRepo = NoOpScanLogForResilience()
            val kbRepo = FailingSaveGraphKBRepo()

            val engine = buildResilienceEngine(
                kbRepo, scanStateRepo, scanLogRepo, ticketKeys
            )
            wireFailingGraphBuilder(engine)
            ...
            assertEquals(
                ...
                "Scan must reach COMPLETED despite graph failures ($projectKey, $ticketCount tickets)"
            )
            assertEquals(
                ticketCount, finalState?.processedCount,
                "processedCount must equal totalTickets ($ticketCount) for $projectKey"
            )
        }
    }

    private fun buildResilienceEngine(
        kbRepo: KBRepository,
        scanStateRepo: ScanStateRepository,
        scanLogRepo: ScanLogRepository,
        ticketKeys: List<String>
    ): BatchScanEngine = BatchScanEngine(
        aiOrchestrator = StubAIForResilience(),
        kbRepository = kbRepo,
        jiraClientProvider = { StubJiraForResilience(ticketKeys) },
        featureNetworkMapper = FeatureNetworkMapper(StubAIAgentForResilience()),
        scanStateRepository = scanStateRepo,
        scanLogRepository = scanLogRepo,
        scope = CoroutineScope(Dispatchers.Default),
        settingsRepository = NoOpSettingsForResilience()
    )

    private fun wireFailingGraphBuilder(engine: BatchScanEngine) {
    ...
    private suspend fun awaitScanDone(
        repo: InMemoryScanStateForResilience,
        projectKey: String,
    ...
    /** KBRepository that throws on saveGraphData to simulate build failures. */
    class FailingSaveGraphKBRepo : KBRepository {
        ...
        override suspend fun saveGraphData(projectKey: String, graph: NetworkGraph): Boolean {
            throw RuntimeException("Simulated graph save failure for $projectKey")
        }
        override suspend fun getGraphData(projectKey: String): NetworkGraph? = null
    }

    class StubAIForResilience : AIOrchestrator {
    ...
    class StubJiraForResilience(private val keys: List<String>) : JiraClient {
    ...
    class NoOpScanLogForResilience : ScanLogRepository {
    ...
    class NoOpSettingsForResilience : SettingsRepository {
